...
export const getStateData = async (req, res, next) => {
    try {
        ...
        QUERY = "SELECT  state,COUNT(id) AS total_count,SUM(pos) AS total_pos,SUM(CASE WHEN CAST(collected_amt AS DECIMAL(10, 2)) >= CAST(emi_amt_fixed AS DECIMAL(10, 2)) THEN pos ELSE 0 END) AS resolved_pos,SUM(CASE WHEN CAST(collected_amt AS DECIMAL(10, 2)) < CAST(emi_amt_fixed AS DECIMAL(10, 2)) THEN pos ELSE 0 END) AS unresolved_pos,SUM(CASE WHEN CAST(collected_amt AS DECIMAL(10, 2)) >= CAST(emi_amt_fixed AS DECIMAL(10, 2)) THEN 1 ELSE 0 END) AS resolved_count,SUM(CASE WHEN CAST(collected_amt AS DECIMAL(10, 2)) < CAST(emi_amt_fixed AS DECIMAL(10, 2)) THEN 1 ELSE 0 END) AS unresolved_count,ROUND(( SUM(CASE WHEN CAST(collected_amt AS DECIMAL(10, 2)) >= CAST(emi_amt_fixed AS DECIMAL(10, 2)) THEN pos ELSE 0 END)* 100.0) / SUM(pos), 2) AS percentage_pos,ROUND(( SUM(CASE WHEN CAST(collected_amt AS DECIMAL(10, 2))< CAST(emi_amt_fixed AS DECIMAL(10, 2)) THEN pos ELSE 0 END)* 100.0) / SUM(pos), 2) AS unresolved_percentage_pos,ROUND((SUM(CASE WHEN CAST(collected_amt AS DECIMAL(10, 2)) >= CAST(emi_amt_fixed AS DECIMAL(10, 2)) THEN 1 ELSE 0 END)*100)/COUNT(id),2) as resolved_percentage_count,ROUND((SUM(CASE WHEN CAST(collected_amt AS DECIMAL(10, 2)) < CAST(emi_amt_fixed AS DECIMAL(10, 2)) THEN 1 ELSE 0 END)*100)/COUNT(id),2) as unresolved_percentage_count FROM `master` WHERE id>0 ";
